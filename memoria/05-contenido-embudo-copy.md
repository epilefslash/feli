...
# Mapa de contenido, embudo, copy y carruseles

> Orden de publicación, bio y destacadas, copy maestro, glosario de edición, y el plan de carruseles.
...
## 34) LOS 15 PRIMEROS POSTEOS SEGÚN NICO — mapeo contra el stock real y ORDEN NUEVO

> **Origen:** la *Hoja de Ruta (Plan de Acción Detallado)* de Método Flow Music pide una composición
> **exacta** para los primeros 15 posteos, dentro del módulo "Perfil Magnético de Nicho" (2 semanas).
> Se auditó contra el inventario real de Feli (1/9). **Este mapeo reemplaza al orden de publicación de
> la sección 9 como plan operativo** — la sección 9 sigue valiendo por sus dos reglas de fondo (no
> encadenar creencias · los vendedores van después de la autoridad), que acá se respetan.

### La composición que pide Nico, y qué teníamos

| Nico pide | Estado real | |
|---|---|---|
| 4 Valor (Reels + **Portadas**) | 8 reels filmados | ✅ sobra · ⚠️ **portadas nunca se trabajaron** |
| 3 Valor (Carruseles) | 4 escritos (sec. 22), 0 diseñados | 🟡 falta diseñarlos |
| **1 Epifanía** | — | ❌ **categoría inexistente** → guion nuevo, sec. 35 |
| 2 StoryTelling (**Foto/Imagen**) | Historia #11, pero es video | ❌ **formato equivocado** → 2 copys nuevos, sec. 36 |
| 2 Autoridad (Video/Foto) | Fijado #12 + #1 + #2 | ✅ |
| **1 De 2 pasos (Imagen)** | — | ❌ **categoría inexistente** → pieza nueva, sec. 37 |
| **2 Testimonios (Video-Entrevistas)** | — | ❌ **el hueco más duro** → guía nueva, sec. 38 |

### 🔴 El hallazgo principal: estamos sobre-indexados en Formato C
**6 de los 8 reels filmados son creencias atacadas.** Nico pide solo 4 posteos de valor en los primeros
15; el resto es storytelling, autoridad, captación y prueba social. Publicando el stock actual tal cual,
la grilla inicial es **un muro de "te mintieron / estás equivocado / falso"** — y sin testimonios ni
epifanía que lo balanceen, eso se lee como reto sostenido, no como autoridad. El Formato C convierte,
pero necesita alrededor las otras cuatro caras.

> **Consecuencia práctica: sobra material de ataque de creencias y falta material de identidad y prueba.**
> No hay que filmar más Formato C por un buen rato. Lo que falta producir es de otro tipo (ver abajo).

### El testimonio: por qué no es imposible aunque no haya cohorte
Feli **tiene alumnos 1 a 1 presenciales hoy**. Esos son testimonios válidos: no del programa grupal,
sí de él como profesor — que es lo que un desconocido necesita para creerle. Se presenta como lo que
es, sin mentir en ningún lado. Detalle completo (las 5 preguntas, cómo se edita, los permisos) en
`memoria/04`, sección 38. **Es el ítem que más tarda porque depende de la agenda de otro: se arranca
primero aunque se publique último.**

---

### 📅 EL ORDEN NUEVO DE LOS 15 (3 posteos por semana)
...
### Lo que queda en stock para la semana 5 en adelante
`#2 acordes funk` (A) · `#3 tocar rápido` (C) · `#5 penta principiantes` (C) · `#7 equipo` (C, falta
grabar guitarras) · `HISTORIA #11` (video de 60 seg, el arco completo) · `Carrusel 3` (5 mentiras) ·
el vendedor **"Como que desaparece"** (sec. 23). Son ~7 posteos = ~2 semanas y media más. **Los
vendedores entran recién acá**, igual que decía la sección 9.

> ⚠️ **"Dejá de mirar tutoriales" (sec. 21) sigue bloqueado y no entra en esta cuenta:** su CTA promete
> "3 licks con el cómo" y ese lead magnet **no existe**. Ver el bloque de guiones de `memoria/00`.

---

### ✍️ Lo que HAY QUE PRODUCIR (y no existe hoy)
...
### Lo que también pide la Hoja de Ruta y no estaba en ningún lado (fuera de contenido)

- **HOTMART** — Nico define la plataforma (nosotros lo teníamos como "pendiente: plataforma del
  videocurso"). **Tiene lead time: ~3 días hábiles de aprobación y puede llevar varios intentos por el
  proceso de verificación.** Arrancarlo temprano, no la semana que se vende.
- **CARTA DE VENTAS** (7 hs en web / 2 hs en Canva / 2 hs en PDF), con sección *Roadmap* + sección
  *"Qué incluye el programa"*. **Las dos secciones ya están escritas** en `memoria/01` §19 — es trabajo
  de formato, no de contenido. Es de lo más barato que queda por hacer.
- **CALENDARIO DE AGENDAS** (3 hs de armado). No teníamos infraestructura de agendamiento: el embudo
  dice "DM → llamada" pero nunca se definió con qué se agenda.
- **ADS** — campaña de tráfico al perfil (15 min) + campaña de mensajes (4 hs). Todo nuestro plan era
  orgánico y nunca se habló de presupuesto. Criterio de lectura de Nico: **a las 6-12 hs ya se sabe si
  un anuncio es malo; hacen falta 5-7 días para confirmar si es bueno** (se sigue optimizando).
- **VALIDACIÓN EN LA BIO** — Nico pide 3 elementos: Promesa de valor / **Validación** / CTA. Nuestra bio
  (sección 11) tiene promesa y CTA, **falta la validación**: el por qué creerte (años tocando, la
  formación, los alumnos). Arreglo de una línea.
- **10 CONVERSACIONES NUEVAS POR DÍA** como punto de partida, organizadas: los 10 arranques a la mañana
  en 30 min + 2 espacios más al día (mediodía y tarde/noche) solo para responder abiertas. Número
  concreto que no teníamos.
- **"Hacé de cuenta que no vas a vender en las primeras 10 llamadas."** Está literal en la Hoja de Ruta.
  Es la regla 97/3 aplicada a Feli mismo, y encaja exacto con su perfil ansioso (ver `memoria/00`).
  Sumado a: **grabar las sesiones de venta para analizarlas**.
- **Tracker de Resultados** — 10 min diarios, sección Anuncios. El archivo existe; falta el hábito.
